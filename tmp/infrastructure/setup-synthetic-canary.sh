...
bindings=$(docker exec -e ACCOUNT_ID="$account_id" -e GROUP_ID="$group_id" "$POSTGRES" sh -ec 'psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" -AtF "|" -c "SELECT (SELECT COUNT(*) FROM account_groups WHERE account_id=${ACCOUNT_ID} AND group_id=${GROUP_ID}),(SELECT COUNT(*) FROM api_keys WHERE group_id=${GROUP_ID} AND deleted_at IS NULL)"')
