# ADMIN_PAYMENT_INTEGRATION_API（中文）
...
### 目标
本文档用于对接外部支付系统（如 `sub2apipay`）与 ExAPI 的 Admin API，覆盖：
- 支付成功后充值
- 用户查询
- 人工余额修正
- 前端购买页参数透传
...
当 ExAPI 打开 `purchase_subscription_url` 或用户侧自定义页面 iframe URL 时，会统一追加：
- `user_id`
- `token`
- `theme`（`light` / `dark`）
...
`token` 属于凭据。不要记录完整 URL，也不要把真实 URL 写入文档、工单、分析系统或
可能发送 Referer 的跳转中。
...
### 6) `doc_url` 配置建议
- 查看链接：`https://github.com/immortal-autumn/ExAPI/blob/main/docs/ADMIN_PAYMENT_INTEGRATION_API.md`
- 下载链接：`https://raw.githubusercontent.com/immortal-autumn/ExAPI/main/docs/ADMIN_PAYMENT_INTEGRATION_API.md`
...
### Purpose
This document describes the minimal ExAPI Admin API surface for external payment integrations (for example, `sub2apipay`), including:
...
When ExAPI opens `purchase_subscription_url` or a user-facing custom page iframe URL, it appends:
- `user_id`
- `token`
- `theme` (`light` / `dark`)
...
- View URL: `https://github.com/immortal-autumn/ExAPI/blob/main/docs/ADMIN_PAYMENT_INTEGRATION_API.md`
- Download URL: `https://raw.githubusercontent.com/immortal-autumn/ExAPI/main/docs/ADMIN_PAYMENT_INTEGRATION_API.md`
