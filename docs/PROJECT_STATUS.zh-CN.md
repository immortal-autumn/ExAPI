...
最新的仅审查提交新增了中英文管理员专用产品范围契约（`bdd4329e3`），让所有受
...
契约和 release contract 均通过。这些提交只在 `revision/exapi-v0.2.1`，**未部署到
OPC，也不包含在 v0.2.10 镜像中**。

最新审查提交 `da3e56788` 将保留的私有安全边界加入管理员设置页；安全页为只读时
...
`revision/exapi-v0.2.1`，**尚未部署到 OPC，也不包含在 v0.2.10 镜像中**。
