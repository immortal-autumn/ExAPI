...
[English（默认）](PAYMENT.md) | 简体中文

> **上游历史参考，不是 ExAPI 功能。** ExAPI 运行在管理员专用模式；客户支付、余额和
> 收银台路由均已退役，受支持的部署模板不会启用它们。本页仅用于历史 schema/迁移
> 背景。**不要将下方的快速开始、服务商、Webhook 或支付流程说明用于线上 ExAPI
> 实例。**

上游 Sub2API 版本曾记录内置支付系统，可支持客户自助充值。本页在 ExAPI 中仅作为
历史兼容参考保留。
