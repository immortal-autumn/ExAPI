...
Commands:
  ...
  up [--recreate]       Create and start the complete ExAPI stack
...
acquire_lock() {
    if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
        ...
        if [[ "${owner_pid}" =~ ^[0-9]+$ ]] && ! kill -0 "${owner_pid}" 2>/dev/null; then
            ...
            die "Another ExAPI Apple container operation is already running."
        fi
    fi
    printf '%s\n' "$$" >"${LOCK_DIR}/pid"
    LOCK_ACQUIRED=true
...
start_app() {
    ...
    if ! wait_for_probe "ExAPI" 180 probe_app; then
        show_failure_logs "${APP_CONTAINER}"
        die "ExAPI did not become ready."
    fi
    if ! wait_for_probe "ExAPI host port" 15 probe_host_app; then
...
cmd_up() {
    ...
    info "ExAPI is available at http://${ACCESS_HOST}:${HOST_PORT}"
}
...
cmd_down() {
    ...
    info "ExAPI stack stopped; persistent volumes were preserved."
}
...
confirm_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        printf 'Delete the ExAPI stack and all persistent data? [y/N] '
    else
        printf 'Delete the ExAPI containers and network, preserving volumes? [y/N] '
    fi
    read -r answer
    [[ "${answer}" == "y" || "${answer}" == "Y" ]]
}
...
cmd_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        ...
        info "ExAPI stack and persistent data deleted."
    else
        info "ExAPI stack deleted; persistent volumes were preserved."
    fi
}
