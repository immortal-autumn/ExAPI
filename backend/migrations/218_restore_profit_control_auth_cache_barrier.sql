-- Migration 215 expanded group authorization-cache invalidation coverage but
-- accidentally dropped the profit-control fields introduced by migration 193.
-- Restore those policy inputs while preserving the complete migration-215
-- trigger body and global-generation invalidation behavior.
...
BEGIN
    ...
    IF TG_OP = 'UPDATE'
       ...
       AND OLD.profit_safety_buffer IS NOT DISTINCT FROM NEW.profit_safety_buffer THEN
        RETURN NEW;
    END IF;
...
-- Existing cache entries may have survived a profit-policy mutation while the
-- migration-215 trigger was active. Establish a new durable global boundary.
INSERT INTO auth_cache_invalidation_outbox (cache_key)
VALUES (repeat('0', 64));
