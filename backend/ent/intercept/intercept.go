...
// The ProtectedSettingFunc type is an adapter to allow the use of ordinary function as a Querier.
type ProtectedSettingFunc func(context.Context, *ent.ProtectedSettingQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f ProtectedSettingFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.ProtectedSettingQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.ProtectedSettingQuery", q)
}

// The TraverseProtectedSetting type is an adapter to allow the use of ordinary function as Traverser.
type TraverseProtectedSetting func(context.Context, *ent.ProtectedSettingQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseProtectedSetting) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseProtectedSetting) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.ProtectedSettingQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.ProtectedSettingQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.ProtectedSettingQuery:
		return &query[*ent.ProtectedSettingQuery, predicate.ProtectedSetting, protectedsetting.OrderOption]{typ: ent.TypeProtectedSetting, tq: q}, nil
	case *ent.ProxyQuery:
