...
type APIKey struct {
	...
	// Versioned purpose-bound HMAC verifier; raw gateway keys are never stored here
	KeyDigest *string `json:"key_digest,omitempty"`
	// Non-secret display prefix for operator identification
...
func (*APIKey) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case apikey.FieldKey, apikey.FieldKeyDigest, apikey.FieldKeyPrefix, apikey.FieldName, apikey.FieldStatus:
...
func (_m *APIKey) String() string {
	...
	builder.WriteString(", ")
	if v := _m.KeyDigest; v != nil {
		builder.WriteString("key_digest=")
