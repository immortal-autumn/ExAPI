...
func latestUsageLogIPsQuery(apiKeyIDs []int64, dialectName string) (string, []any) {
	if dialectName == dialect.Postgres {
		...
		CROSS JOIN LATERAL (
			...
			LIMIT 1
		) AS latest`, []any{postgres.Array(apiKeyIDs)}
	}
...
func (r *apiKeyRepository) IncrementQuotaUsedAndGetState(ctx context.Context, id int64, amount float64) (*service.APIKeyQuotaUsageState, error) {
	query := `
		...
		RETURNING quota_used, quota, user_id, key, status
	`

	state := &service.APIKeyQuotaUsageState{}
	if err := scanSingleRow(ctx, r.sql, query, []any{amount, service.StatusAPIKeyQuotaExhausted, id}, &state.QuotaUsed, &state.Quota, &state.UserID, &state.Key, &state.Status); err != nil {
