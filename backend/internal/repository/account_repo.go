...
type accountRepository struct {
	...
	protector *accountCredentialProtector
...
func NewAccountRepository(client *dbent.Client, sqlDB *sql.DB, schedulerCache service.SchedulerCache) (service.AccountRepository, error) {
	keyrings, err := config.LoadExternalKeyringsFromEnv()
	if err != nil {
		return nil, err
	}
	protector, err := newAccountCredentialProtector(keyrings.DataEncryption)
	if err != nil {
		return nil, err
	}
	return newAccountRepositoryWithSQLAndProtector(client, sqlDB, schedulerCache, protector), nil
}

// NewAdminAccountRepository exposes atomic admin account operations while
// preserving the same fail-closed credential protector as the main repository.
func NewAdminAccountRepository(client *dbent.Client, sqlDB *sql.DB, schedulerCache service.SchedulerCache) (service.AdminAccountRepository, error) {
	keyrings, err := config.LoadExternalKeyringsFromEnv()
	if err != nil {
		return nil, err
	}
...
// newAccountRepositoryWithSQL 是内部兼容构造函数。凭据写入会失败关闭，
// 需要凭据读写的测试应使用 newAccountRepositoryWithSQLAndProtector。
func newAccountRepositoryWithSQL(client *dbent.Client, sqlq sqlExecutor, schedulerCache service.SchedulerCache) *accountRepository {
	return newAccountRepositoryWithSQLAndProtector(client, sqlq, schedulerCache, nil)
}

func newAccountRepositoryWithSQLAndProtector(client *dbent.Client, sqlq sqlExecutor, schedulerCache service.SchedulerCache, protector *accountCredentialProtector) *accountRepository {
	return &accountRepository{client: client, sql: sqlq, schedulerCache: schedulerCache, protector: protector}
}
...
func (r *accountRepository) createAccountRecord(ctx context.Context, client *dbent.Client, account *service.Account) error {
	...
	if r.protector == nil {
		return errors.New("data-encryption keyring is required for account credentials")
	}
	...
	sealedCredentials, err := r.protector.seal(created.ID, account.Credentials)
	if err != nil {
		return err
	}
	if _, err := client.Account.UpdateOneID(created.ID).SetCredentials(sealedCredentials).Save(ctx); err != nil {
		return translatePersistenceError(err, service.ErrAccountNotFound, nil)
	}
...
// UpdateWithAccountBillingSettings applies an admin edit while preserving a
// concurrently probe-synchronized multiplier unless the request explicitly
...
func (r *accountRepository) updateLockedAccount(ctx context.Context, client *dbent.Client, account *service.Account, explicitProbeEnabled, explicitRateSyncEnabled *bool, explicitRateMultiplier *float64) (*dbent.Account, error) {
	...
	builder := client.Account.UpdateOneID(account.ID).
		...
		SetPlatform(account.Platform).
		SetType(account.Type).
		SetCredentials(sealedCredentials).
		SetExtra(extra).
		SetConcurrency(account.Concurrency).
		SetPriority(account.Priority).
		SetStatus(account.Status).
...
func (r *accountRepository) lockAndMergeAccountProbeExtra(ctx context.Context, client *dbent.Client, account *service.Account, explicitProbeEnabled *bool) (map[string]any, error) {
	...
	plain, _, err := r.protector.open(account.ID, stored)
	if err != nil {
		return nil, err
	}
	plainJSON, err := json.Marshal(normalizeJSONMap(plain))
	if err != nil {
		return nil, err
	}
	identityUnchanged := metadataUnchanged && string(plainJSON) == string(credentials)
...
func (r *accountRepository) ListOAuthRefreshCandidatePage(ctx context.Context, options service.OAuthRefreshPageOptions) (*service.OAuthRefreshCandidatePage, error) {
	...
	//
	// schedulable=false 仅表示暂停调度，不表示 OAuth 凭据永久失效。
	// 暂停但仍 active 的账号也必须继续刷新令牌，否则管理端用量探测（不
	// 经过请求路径 token provider）会在令牌过期后误报需要重新授权。真正
	// 的失败由 status=active 过滤和 retry cooldown 共同限制，刷新成功只
	// 更新凭据，不会把账号自动恢复为可调度。
	query := `
		SELECT id
		FROM accounts
...
func (r *accountRepository) BatchUpdateLastUsed(ctx context.Context, updates map[int64]time.Time) error {
	...
	args = append(args, postgres.Array(ids))
...
func (r *accountRepository) ListSchedulableCapacityByGroupIDs(ctx context.Context, groupIDs []int64) ([]service.GroupAccountCapacityRow, error) {
	...
	`, postgres.Array(groupIDs), service.StatusActive, time.Now())
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
...
func (r *accountRepository) BulkUpdate(ctx context.Context, ids []int64, updates service.AccountBulkUpdate) (int64, error) {
	...
	// Credential updates must decrypt, merge, and re-seal each row because the
	// envelope is purpose-bound to the account ID. They are still applied in the
	// single final UPDATE statement through a per-ID CASE expression.
	...
	if len(updates.Credentials) > 0 {
		...
		for _, id := range ids {
			...
			caseParts = append(caseParts, "WHEN $"+itoa(idx)+" THEN $"+itoa(idx+1)+"::jsonb")
			args = append(args, id, payload)
			idx += 2
		}
		if len(caseParts) > 0 {
	...
	if len(updates.Extra) > 0 || len(ollamaGroupIdentityChangedIDs) > 0 || ollamaProxyIdentityChanged != "" || accountTestProbeIdentityChanged {
		...
		if len(ollamaGroupIdentityChangedIDs) > 0 {
			...
			args = append(args, postgres.Array(ollamaGroupIdentityChangedIDs))
			idx++
		}
...
func (r *accountRepository) sealAccountCredentials(id int64, credentials map[string]any) (map[string]any, error) {
	if r == nil || r.protector == nil {
		return nil, errors.New("data-encryption keyring is required for account credentials")
	}
	return r.protector.seal(id, credentials)
}
