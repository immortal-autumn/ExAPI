...
func TestLockAndMergeAccountProbeExtraUsesCurrentDatabaseSnapshot(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			stored, err := protector.seal(27, map[string]any{"api_key": "sk-test"})
			require.NoError(t, err)
			...
			got, err := repo.lockAndMergeAccountProbeExtra(context.Background(), client, account, nil)
			require.NoError(t, err)
...
func TestUpdateCredentialsAtomicallyClearsProbeForOpenAIAPIKeyIdentityChange(t *testing.T) {
	...
	t.Cleanup(func() { _ = client.Close() })
	protector := mustAccountCredentialProtectorForTest(t)
	stored, err := protector.seal(27, map[string]any{"api_key": "sk-old"})
	require.NoError(t, err)
	storedJSON, err := json.Marshal(stored)
	require.NoError(t, err)
	...
	mock.ExpectQuery(`(?s)SELECT .* FROM "accounts".*WHERE .*"id" = \$1`).
		WithArgs(int64(27)).
		WillReturnRows(updatedAccountRows(27, storedJSON, `{}`))
	...
	mock.ExpectBegin()
	mock.ExpectExec(`(?s)UPDATE accounts.*credentials = \$1::jsonb.*- 'upstream_billing_probe'.*- 'account_test_probe'`).
...
func TestUpdateWithUpstreamBillingProbeEnabledRollsBackWhenOutboxFails(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)SELECT .* FROM "accounts" WHERE "id" = \$1`).
		WithArgs(int64(27)).
		WillReturnRows(updatedAccountRows(27, storedJSON, `{"upstream_billing_probe_enabled":false}`))
