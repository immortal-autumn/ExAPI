...
INSERT INTO batch_image_jobs (
    ...
    pricing_snapshot_version, api_key_quota_tracked, api_key_rate_limit_tracked,
    account_type_snapshot, account_quota_tracked,
    currency, hold_id,
...
RETURNING `+batchImageJobColumns,
		...
		params.PricingSnapshotVersion, params.APIKeyQuotaTracked, params.APIKeyRateLimitTracked,
		params.AccountTypeSnapshot, params.AccountQuotaTracked,
...
func scanBatchImageJob(row rowScanner) (*service.BatchImageJob, error) {
	...
	err := row.Scan(
		...
		&job.PricingSnapshotVersion, &job.APIKeyQuotaTracked, &job.APIKeyRateLimitTracked,
		&job.AccountTypeSnapshot, &job.AccountQuotaTracked,
		&job.Currency, &holdID,
