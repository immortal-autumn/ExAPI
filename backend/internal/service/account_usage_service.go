...
func (s *AccountUsageService) getAntigravityUsage(ctx context.Context, account *Account, forceProbe bool) (*UsageInfo, error) {
	if s.antigravityQuotaFetcher == nil || !s.antigravityQuotaFetcher.CanFetch(account) {
		now := time.Now()
		return &UsageInfo{UpdatedAt: &now}, nil
	}

	// 1. Check the cache unless the caller explicitly requested a live probe.
	if !forceProbe {
	...
	if forceProbe {
		// A forced refresh must not join an already-running ordinary cache fill,
		// because that flight is allowed to return the cached snapshot.
		flightKey += ":force"
	}
	result, flightErr, _ := s.cache.antigravityFlight.Do(flightKey, func() (any, error) {
		// Recheck the cache for ordinary callers; a forced probe deliberately
		// bypasses it so the UI's Refresh action reaches the provider.
		if !forceProbe {
			if cached, ok := s.cache.antigravityCache.Load(account.ID); ok {
				if cache, ok := cached.(*antigravityUsageCache); ok {
					ttl := antigravityCacheTTL(cache.usageInfo)
					if time.Since(cache.timestamp) < ttl {
						usage := cache.usageInfo
						// Recalculate RemainingSeconds to avoid stale countdowns.
