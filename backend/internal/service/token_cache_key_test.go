...
func TestAntigravityTokenCacheKeySeparatesAccountsWithSharedProjectID(t *testing.T) {
	first := &Account{ID: 206, Credentials: map[string]any{"project_id": "shared-project"}}
	second := &Account{ID: 207, Credentials: map[string]any{"project_id": "shared-project"}}

	require.NotEqual(t, AntigravityTokenCacheKey(first), AntigravityTokenCacheKey(second))
	require.Equal(t, "ag:account:206", AntigravityTokenCacheKey(first))
	require.Equal(t, "ag:account:207", AntigravityTokenCacheKey(second))
}
