...
func (s *ContentModerationService) moderationHTTPClient(ctx context.Context, cfg *ContentModerationConfig) (*http.Client, error) {
	...
	// GetClient returns a shared client. Copy it before installing the strict
	// redirect checker so other callers cannot weaken or inherit this policy.
	moderationClient := *client
	moderationClient.CheckRedirect = contentModerationOutboundPolicy.RedirectChecker(false, 10)
