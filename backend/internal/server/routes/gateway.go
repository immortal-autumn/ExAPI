...
func registerGatewayRoutes(
	...
	gateway.Use(requireGroupAnthropic)
	{
		...
		gateway.POST(routecontract.ResponsesPath, func(c *gin.Context) {
			if isOpenAIResponsesCompatibleGatewayPlatform(c) {
		...
		gateway.POST(routecontract.ResponsesPath+"/*subpath", guardResponsesSubpath(func(c *gin.Context) {
			if isOpenAIResponsesCompatibleGatewayPlatform(c) {
				h.OpenAIGateway.Responses(c)
				return
			}
		...
		gateway.GET(routecontract.ResponsesPath, func(c *gin.Context) {
			h.OpenAIGateway.ResponsesWebSocket(c)
		})
		// OpenAI Chat Completions API: auto-route based on group platform
		gateway.POST(routecontract.ChatCompletionsPath, func(c *gin.Context) {
			if isOpenAIResponsesCompatibleGatewayPlatform(c) {
		...
		gateway.POST(routecontract.EmbeddingsPath, textBodyLimit, func(c *gin.Context) {
		...
		gateway.POST(routecontract.ImagesGenerationsPath, imagesHandler)
		gateway.POST(routecontract.ImagesEditsPath, imagesHandler)
		...
		gateway.POST(routecontract.VideosGenerationsPath, videoGenerationHandler)
		gateway.GET(routecontract.VideosStatusPath, videoStatusHandler)
	...
	r.POST(routecontract.ResponsesPath, bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, responsesHandler)
	r.POST(routecontract.ResponsesPath+"/*subpath", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, guardResponsesSubpath(responsesHandler))
	...
	r.GET(routecontract.ResponsesPath, bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, func(c *gin.Context) {
		h.OpenAIGateway.ResponsesWebSocket(c)
	})
	...
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic)
	{
		...
		codexDirect.POST(routecontract.ResponsesPath, responsesHandler)
		codexDirect.POST(routecontract.ResponsesPath+"/*subpath", guardResponsesSubpath(responsesHandler))
		codexDirect.POST("/alpha/search", textBodyLimit, h.OpenAIGateway.AlphaSearch)
		codexDirect.GET(routecontract.ResponsesPath, func(c *gin.Context) {
			h.OpenAIGateway.ResponsesWebSocket(c)
		})
		codexDirect.GET("/models", h.OpenAIGateway.CodexModels)
	}
	// OpenAI Chat Completions API（不带v1前缀的别名）— auto-route based on group platform
	r.POST(routecontract.ChatCompletionsPath, bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, func(c *gin.Context) {
		if isOpenAIResponsesCompatibleGatewayPlatform(c) {
			h.OpenAIGateway.ChatCompletions(c)
			return
		}
	...
	r.POST(routecontract.EmbeddingsPath, textBodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, func(c *gin.Context) {
		if !isOpenAIOnlyEndpointGatewayPlatform(c) {
			service.MarkOpsClientBusinessLimited(c, service.OpsClientBusinessLimitedReasonLocalFeatureGate)
			c.JSON(http.StatusNotFound, gin.H{
				"error": gin.H{
	...
	r.POST(routecontract.ImagesGenerationsPath, bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, imagesHandler)
	r.POST(routecontract.ImagesEditsPath, bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, imagesHandler)
	...
	r.POST(routecontract.VideosGenerationsPath, bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, videoGenerationHandler)
	...
	r.GET(routecontract.VideosStatusPath, bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, videoStatusHandler)
