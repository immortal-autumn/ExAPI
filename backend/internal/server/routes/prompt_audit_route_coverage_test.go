...
func TestEveryGatewayPOSTRouteIsClassifiedForPromptAuditCoverage(t *testing.T) {
	...
	normalizedRouteSource := strings.NewReplacer(
		`routecontract.ResponsesPath+"/*subpath"`, `"/responses/*subpath"`,
		"routecontract.ResponsesPath", `"/responses"`,
		"routecontract.ChatCompletionsPath", `"/chat/completions"`,
		"routecontract.EmbeddingsPath", `"/embeddings"`,
		"routecontract.ImagesGenerationsPath", `"/images/generations"`,
		"routecontract.ImagesEditsPath", `"/images/edits"`,
		"routecontract.VideosGenerationsPath", `"/videos/generations"`,
	).Replace(string(routeSource))
	pattern := regexp.MustCompile(`(?:gateway|gemini|r|codexDirect|antigravityV1|antigravityV1Beta)\.POST\("([^"]+)"`)
	matches := pattern.FindAllStringSubmatch(normalizedRouteSource, -1)
...
func TestResponsesWebSocketHasFirstAndSubsequentTurnPromptGates(t *testing.T) {
	routeSource, err := os.ReadFile("gateway.go")
	require.NoError(t, err)
	normalizedRouteSource := strings.ReplaceAll(string(routeSource), "routecontract.ResponsesPath", `"/responses"`)
	require.GreaterOrEqual(t, strings.Count(normalizedRouteSource, `.GET("/responses"`), 2)
