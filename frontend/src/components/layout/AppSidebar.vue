...
// Legacy icon constants remain source-compatible with downstream themes while
// the private route table intentionally has no commercial navigation entries.
void GiftIcon
void UsersIcon
void CreditCardIcon
void RechargeSubscriptionIcon
void BellIcon
void TicketIcon
void OrderIcon
void OrderListIcon
...
// Apply operator feature flags and private-control hints.
function finalizeNav(items: NavItem[]): NavItem[] {
  const visible = applyFeatureFlags(items).filter((item) => !item.children || item.children.length > 0)
