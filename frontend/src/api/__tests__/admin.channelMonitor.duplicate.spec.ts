...
describe('admin channel monitor duplicate API', () => {
  ...
  it('sends a stable idempotency key with the duplicate request', async () => {
    ...
    expect(post).toHaveBeenCalledWith('/admin/channel-monitors/42/duplicate', undefined, {
      headers: {
        'Idempotency-Key': 'channel-monitor-duplicate-operator-42-11111111-1111-4111-8111-111111111111',
      },
    })
  ...
  it('keeps the operation key scoped to the singleton operator when browser identity changes', async () => {
    ...
    expect(post.mock.calls[1][2].headers).toEqual(firstHeaders)
    expect(sessionStorage.getItem('sub2api:admin:channel-monitor-duplicate:operator:55')).toBeNull()
  })
