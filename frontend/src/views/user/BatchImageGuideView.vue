...
import { resolveBatchImageAgentInstruction } from '@/utils/batchImageAgentInstruction'
...
const endpointBase = computed(() => {
  ...
  return '<your ExAPI control-plane endpoint>'
})
...
const agentInstruction = computed(() => {
  // The raw locale message contains JSON braces, so avoid vue-i18n's deeply
  // inferred interpolation signature and resolve the endpoint separately.
  const template = (tm as unknown as (key: string) => unknown)('batchImage.agentInstruction')
  return resolveBatchImageAgentInstruction(template, endpointBase.value)
})
