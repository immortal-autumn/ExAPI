    ...
    <div class="space-y-6 p-6">
      <div class="grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,1fr)_220px]">
        ...
        <div>
          ...
          <input v-model="form.login_agreement_updated_at" type="date" class="input" />
          <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
      ...
      <div>
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            type="button"
            data-test="add-login-agreement-document"
            ...
            <div class="mb-3 flex items-center justify-between gap-3">
              <div class="flex min-w-0 items-center gap-3">
                <span class="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-md bg-gray-100 text-gray-700 dark:bg-dark-700 dark:text-dark-200">
                  <Icon :name="documentIcon(index)" size="sm" />
                </span>
                <div class="min-w-0">
              ...
              <button
                type="button"
                :data-test="`remove-login-agreement-document-${index}`"
...
type AgreementIconName = 'document' | 'shield' | 'globe' | 'cog'

interface AgreementSettingsFormSlice {
...
defineProps<{
  form: AgreementSettingsFormSlice
  localText: (zh: string, en: string) => string
  loginAgreementRoutePath: (doc: LoginAgreementDocument, index: number) => string
  addLoginAgreementDocument: () => void
  removeLoginAgreementDocument: (index: number) => void
}>()

function documentIcon(index: number): AgreementIconName {
  if (index === 1) return 'shield'
  if (index === 2) return 'globe'
