export default {
    accounts: {
      ...
      upstreamBilling: {
        trustWarning: '此倍率由上游站点针对当前 API Key 自行声明。ExAPI 无法验证该值是否与实际扣费一致；上游站点或中间代理可能返回伪造、过期或被篡改的数据。请结合账单、余额变化和实际用量自行核验。',
      ...
      types: {
        ...
        grokSsoFiles: 'SSO 文件',
        grokSsoFilesDesc: '导入 Web SSO 文件',
      ...
      oauth: {
        ...
        grok: {
          ...
          buildFilesHint: '上传 grok2api Grok Build 导出文件（.json/.jsonl/.txt），解析出的 refresh_token 会填入上方文本框供确认。',
          buildFilesParsed: '已解析 {count} 个 refresh token',
          buildFilesEmpty: '所选文件中没有找到 refresh token。',
          buildFilesReadFailed: '读取所选文件失败。',
          buildFilesTooManyFiles: '每次最多选择 10 个文件。',
          buildFilesFileTooLarge: '每个文件不得超过 10 MB。',
          buildFilesTotalTooLarge: '所选文件总大小不得超过 25 MB。',
          buildFilesTooManyTokens: '所选文件包含超过 {limit} 个 refresh token。',
          ssoCookieAuth: 'SSO Cookie / 文件导入',
          ...
          ssoFilesSelect: '选择 SSO 文件',
          ssoFilesHint: '上传 .txt、.json、.jsonl、.csv 或 .sso 文件，内容为 Grok Web SSO key；解析后会填入上方文本框供确认。',
          ssoFilesParsed: '已解析 {count} 个 SSO key',
          ssoFilesEmpty: '所选文件中没有找到 Grok SSO key。',
          ssoFilesReadFailed: '读取所选文件失败。',
          ssoFilesTooManyFiles: '每次最多选择 10 个文件。',
          ssoFilesFileTooLarge: '每个文件不得超过 10 MB。',
          ssoFilesTotalTooLarge: '所选文件总大小不得超过 25 MB。',
          ssoFilesTooManyTokens: '所选文件包含超过 {limit} 个 SSO key。',
          convertingSSO: '转换中...',
      ...
      testHttpError: '请求失败（HTTP {status}）',
      testNoResponseBody: '服务器未返回响应内容',
      testErrorLine: '错误：{message}',
