export default {
    settings: {
      ...
      privateControlPlane: {
        runtimeTitle: '管理员运行时',
        runtimeDescription: '私有网关接入、监控和路由控制。',
        monitoringEnabled: '运维监控',
        realtimeMonitoringEnabled: '实时监控',
        channelMonitorEnabled: '渠道监控',
        riskControlEnabled: '内容风控',
        allowUngroupedKeyScheduling: '允许未分组密钥调度',
        trustForwardedIp: '信任转发 IP 用于 API 密钥 ACL',
        monitorInterval: '监控间隔（秒）',
        metricsInterval: '指标间隔（秒）',
        forwardedHeaders: '转发的客户端 IP 请求头（每行一个）',
        fallbackAnthropic: 'Anthropic 回退模型',
        fallbackOpenai: 'OpenAI 回退模型',
        fallbackGemini: 'Gemini 回退模型',
        fallbackAntigravity: 'Antigravity 回退模型',
        routingTitle: '网关路由',
        routingDescription: '公共监听器的提供商回退和请求规范化设置。',
        modelFallback: '启用模型回退',
        identityPatch: '启用身份补丁',
        fingerprintUnification: '统一客户端指纹',
        clientDatelineNormalization: '规范化客户端日期线',
        metadataPassthrough: '向上游透传元数据',
        openaiAdvancedScheduler: 'OpenAI 高级调度器',
        identityPatchPrompt: '身份补丁提示词',
        compatibilityTitle: '客户端兼容性',
        compatibilityDescription: '可选的客户端版本范围。留空表示不限制对应边界。',
        minClaudeCodeVersion: '最低 Claude Code 版本',
        maxClaudeCodeVersion: '最高 Claude Code 版本',
        minCodexVersion: '最低 Codex 版本',
        maxCodexVersion: '最高 Codex 版本',
        allowCodexAppServerClients: '允许 Codex app-server 客户端',
      },
      tabs: {
        general: '通用设置',
        agreement: '登录条款',
      ...
      security: {
        privateBoundaryTitle: '管理员专用边界',
        privateBoundaryDescription: '管理员身份由专用控制监听器绑定到允许的 WireGuard 对等节点。浏览器不会使用密码、Passkey、一次性验证码、会话令牌、OAuth 登录或 CAPTCHA 挑战。',
        requestProtectionsTitle: '请求保护',
        controlMarker: '每个控制请求都携带明确的 ExAPI 控制标记。',
        requestValidation: '服务端会校验 Host、Origin、Fetch Metadata 和管理员对等节点地址。',
        controlWebSockets: '控制 WebSocket 会协商 exapi-control 子协议。',
      ...
      linuxdo: {
        ...
        description: '配置 LinuxDo Connect OAuth，用于 ExAPI 用户登录',
      ...
      dingtalk: {
        title: '钉钉登录',
        description: '配置钉钉 OAuth，用于 ExAPI 用户登录',
      ...
      gatewayForwarding: {
        ...
        openaiCodexUserAgentHint: '出站统一使用的完整 Codex User-Agent。填写后首段和尾部版本号会同步为后端当前生效版本；留空使用标准 codex-tui 身份。',
      ...
      payment: {
        ...
        easypayCustomMethodsHint: '添加当前易支付服务商额外支持的支付方式。支付方式会记录到 ExAPI 订单中，上游 type 会作为易支付 type 参数提交。',
