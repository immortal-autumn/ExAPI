export default {
  ...
  keys: {
    ...
    secretDisplayHint: '完整 API 密钥仅在创建成功后立即显示一次。已有密钥只显示前缀且无法恢复；如果错过，请删除后重新创建。',
    createdKeyTitle: '保存新 API 密钥',
    createdKeyWarning: '此密钥仅显示一次且无法恢复。关闭此窗口前，请将其复制到安全位置。',
    copyCreatedKey: '复制密钥',
    createdKeySaved: '我已保存密钥',
    keySecretUnavailable: '密钥已创建，但响应未返回一次性密钥。请删除该密钥后重新创建，并在关闭窗口前复制完整密钥。',
    ...
    useKeyModal: {
      ...
      grok: {
        description: '配置 Grok Build、Claude Code、Codex 或 OpenCode，让请求通过当前 ExAPI Grok 分组发送。',
        claudeDescription: '配置 Claude Code，让 Messages API 请求通过当前 ExAPI Grok 分组发送。',
        codexDescription: '配置 Codex，让 Responses API 请求通过当前 ExAPI Grok 分组发送。',
