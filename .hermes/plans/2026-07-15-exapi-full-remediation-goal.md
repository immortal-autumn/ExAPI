...
- [x] **SEC-006 Redis authentication:** bundled private deployments require and preserve a password, application/health checks authenticate, backend private-mode validation fails closed, and an isolated `--network none` Redis 8 container rejected unauthenticated access while accepting the configured credential. External Redis deployment paths pass credentials but cannot enforce server policy.
